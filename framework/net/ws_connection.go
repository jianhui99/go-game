		...
		case message, ok := <-c.WriteChan:
			if !ok {
				if err := c.Conn.WriteMessage(websocket.CloseMessage, []byte{}); err != nil {
					logs.Error("write closeMessage failed,err:%v", err)
				}
				return
			}

			if err := c.Conn.WriteMessage(websocket.BinaryMessage, message); err != nil {
				logs.Info("write message failed,err:%v", err)
			}
		case <-ticker.C:
			if err := c.Conn.SetWriteDeadline(time.Now().Add(WriteWait)); err != nil {
				logs.Error("write ping message SetWriteDeadline err:%v", err)
			}

			if err := c.Conn.WriteMessage(websocket.PingMessage, nil); err != nil {
				logs.Error("write ping message failed,err:%v", err)
			}
		}
	}
}
...
func (c *WsConnection) readMessage() {
	...
	if err := c.Conn.SetReadDeadline(time.Now().Add(pongWait)); err != nil {
		logs.Error("set read deadline failed,err:%v", err)
		return
	}
	for {
		...
		if messageType == websocket.BinaryMessage {
			...
			logs.Error("unknown message type: %v", messageType)
		}
	}
}
